...
uint8_t MoveMotor(motor_controller *mc, uint8_t dir,uint32_t step) {
	...
	if(dir == 1) {
		if(mc->name == 0) {
			mc->step_remain = ((mc->c_motor->currentStep + step) < 15000) ? step: (15000 - mc->c_motor->currentStep);
		}
		else if (mc->name == 1) {
			mc->step_remain = ((mc->c_motor->currentStep + step) < 15000) ? step: (15000 - mc->c_motor->currentStep);
		}
...
uint8_t MoveHome(motor_controller *mc_x,motor_controller *mc_y) {
	...
	MoveMotor(mc_x,0,mc_x->c_motor->currentStep);
	MoveMotor(mc_y,0,mc_y->c_motor->currentStep);
	return 1;
};
...
uint8_t Reset(motor_controller *mc){ 
	...
	mc->c_motor->direction = 1;
	mc->c_motor->speed = 0;
	mc->c_motor->maxSpeed = 20000;
	return 1;
};
