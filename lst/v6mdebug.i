...
{
    USB_SendBackData((u32Errno == 0 ? 0 : 1), pu8Buffer, u32Len);
...
void V6M_ProcessCommand(const uint8_t *pu8Buffer, uint32_t u32Len)
{
    static uint8_t au8CmdBuffer[(64-2)];
...
uint32_t uMotor_Pause(uint8_t mc_name) {
	...
	if(mc_name == 0) PauseMotor(&mcX);
	else if(mc_name == 1) PauseMotor(&mcY);
